...
export default function StudentIdentifiersSection({
  ...
  const validateField = (name: keyof FieldErrors, value: string): string => {
    switch (name) {
      case 'studentPen':
        if (value && value.length !== 11) return 'Student PEN must be exactly 11 characters';
        return '';
      case 'apparNumber':
        if (value && value.length !== 12) return 'APPAR Number must be exactly 12 characters';
        return '';
      case 'eShikshakoshNumber':
        if (value && value.length !== 15) return 'E-Shikshakosh Number must be exactly 15 characters';
        return '';
      case 'studentPhone':
        if (value && !/^\d{10}$/.test(value)) return 'Student Phone must be exactly 10 digits';
        return '';
      case 'studentEmail':
      ...
      <CardContent className="space-y-4">
        <div className="grid md:grid-cols-2 gap-4">
          ...
          <div className="space-y-1">
            <Label htmlFor="studentPen">
              ...
              <span className="ml-1 text-xs text-muted-foreground">(exactly 11 characters)</span>
            </Label>
            <Input
              id="studentPen"
              ...
              maxLength={11}
              disabled={disabled}
              className={errors.studentPen ? 'border-destructive focus-visible:ring-destructive' : ''}
            />
            {errors.studentPen && (
          ...
          <div className="space-y-1">
            <Label htmlFor="apparNumber">
              APPAR Number
              <span className="ml-1 text-xs text-muted-foreground">(exactly 12 characters)</span>
            </Label>
            <Input
              id="apparNumber"
              ...
              maxLength={12}
              disabled={disabled}
              className={errors.apparNumber ? 'border-destructive focus-visible:ring-destructive' : ''}
            />
            {errors.apparNumber && (
        ...
        <div className="space-y-1">
          <Label htmlFor="eShikshakoshNumber">
            E-Shikshakosh Number
            <span className="ml-1 text-xs text-muted-foreground">(exactly 15 characters)</span>
          </Label>
          <Input
            ...
            maxLength={15}
            disabled={disabled}
            className={errors.eShikshakoshNumber ? 'border-destructive focus-visible:ring-destructive' : ''}
          />
          {errors.eShikshakoshNumber && (
            <p className="text-xs text-destructive">{errors.eShikshakoshNumber}</p>
          )}
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-4">
          ...
          <div className="space-y-1">
            <Label htmlFor="studentPhone">
              Student Phone
              <span className="ml-1 text-xs text-muted-foreground">(10 digits)</span>
            </Label>
            <Input
              id="studentPhone"
              type="tel"
              ...
              maxLength={10}
              disabled={disabled}
              className={errors.studentPhone ? 'border-destructive focus-visible:ring-destructive' : ''}
            />
            {errors.studentPhone && (
          ...
          <div className="space-y-1">
            ...
            <Input
              ...
              disabled={disabled}
              className={errors.studentEmail ? 'border-destructive focus-visible:ring-destructive' : ''}
            />
            {errors.studentEmail && (
